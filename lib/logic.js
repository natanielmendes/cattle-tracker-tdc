 ...
 * @transaction
 */
async function clear(clear) {
    const NS = 'org.cattle.tracker';

    let animalRegistry = await getAssetRegistry(NS + '.Animal')
    let animals = await animalRegistry.getAll();
    await animalRegistry.removeAll(animals);
}

/**
 * Transfer animal transaction
 * @param {org.cattle.tracker.Reset} reset
 * @transaction
 */
async function reset(reset) {
    const NS = 'org.cattle.tracker';
